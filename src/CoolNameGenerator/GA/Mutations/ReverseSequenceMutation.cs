        ...
        protected override void PerformMutate(IChromosome chromosome, float probability)
        {
            if (chromosome.Length < 3)
            {
                throw new MutationException(this,
                    "A chromosome should have, at least, 3 genes. {0} has only {1} gene.".With(
                        chromosome.GetType().Name, chromosome.Length));
            }

            if (FastRandom.Next() <= probability)
            {
                var indexes = FastRandom.GetUniqueInts(2, 0, chromosome.Length).OrderBy(i => i).ToArray();
                var firstIndex = indexes[0];
                var secondIndex = indexes[1];

                var revertedSequence =
                    chromosome.GetGenes().Skip(firstIndex).Take(secondIndex - firstIndex + 1).Reverse().ToArray();
