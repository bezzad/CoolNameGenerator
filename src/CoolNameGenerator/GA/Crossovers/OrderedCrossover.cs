...
namespace CoolNameGenerator.GA.Crossovers
{
    ...
    ///             From the replaced portion on, the rest is filled up by the remaining genes, where already present genes are
    ///             omitted and the order is preserved.
    ///             <see href="http://en.wikipedia.org/wiki/Crossover_(genetic_algorithm)#Crossover_for_Ordered_Chromosomes">
    ///                 Crossover
    ...
    ///             The OX1 exploits a property of the path representation, that the order of cities (not their positions) are
    ///             important.
    ...
    public sealed class OrderedCrossover : CrossoverBase
    {
        ...
        protected override IList<IChromosome> PerformCross(IList<IChromosome> parents)
        {
            ...
            var middleSectionIndexes = FastRandom.GetUniqueInts(2, 0, firstParent.Length);
        ...
        private static IChromosome CreateChild(IChromosome firstParent, IChromosome secondParent,
            int middleSectionBeginIndex, int middleSectionEndIndex)
        {
