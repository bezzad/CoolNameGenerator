...
namespace CoolNameGenerator.GA.Crossovers
{
    ...
    ///         A portion of one parent’s string is mapped onto a portion of the other parent’s string and the remaining
    ///         information is exchanged.
    ///         <see href="http://lev4projdissertation.googlecode.com/svn-history/r100/trunk/reading/read/aiRev99.pdf">
    ///             Genetic
    ...
    public class PartiallyMappedCrossover : CrossoverBase
    {
        ...
        /// <summary>
        ///     Initializes a new instance of the <see cref="PartiallyMappedCrossover" /> class.
        /// </summary>
        ...
        protected override IList<IChromosome> PerformCross(IList<IChromosome> parents)
        {
            ...
            var cutPointsIndexes = FastRandom.GetUniqueInts(2, 0, parent1.Length);
            var firstCutPointIndex = cutPointsIndexes[0];
            var secondCutPointIdnex = cutPointsIndexes[1];

            var parent1Genes = parent1.GetGenes();
            var parent1MappingSection =
                parent1Genes.Skip(firstCutPointIndex).Take(secondCutPointIdnex - firstCutPointIndex + 1).ToArray();

            var parent2Genes = parent2.GetGenes();
            var parent2MappingSection =
                parent2Genes.Skip(firstCutPointIndex).Take(secondCutPointIdnex - firstCutPointIndex + 1).ToArray();
            ...
            for (var i = 0; i < length; i++)
            {
                ...
                var geneForoffspring1 = GetGeneNotInMappingSection(parent1Genes[i], parent2MappingSection,
                    parent1MappingSection);
                offspring1.ReplaceGene(i, geneForoffspring1);

                var geneForoffspring2 = GetGeneNotInMappingSection(parent2Genes[i], parent1MappingSection,
                    parent2MappingSection);
        ...
        private Gene GetGeneNotInMappingSection(Gene candidateGene, Gene[] mappingSection,
            Gene[] otherParentMappingSection)
        {
            ...
            return indexOnMappingSection != null
                ? GetGeneNotInMappingSection(otherParentMappingSection[indexOnMappingSection.Index], mappingSection,
                    otherParentMappingSection)
                : candidateGene;
        }
